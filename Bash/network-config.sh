...
#this is to see the hostname
hn=$(hostname)

#this is for getting the computers ip address the awk is there for picking out just the ip address
ln=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')

#this is to get the hostname from hosts file
lh=$(getent hosts $(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')| awk '{print $2}')

#is for getting the external ip of the network by going online to this website
ei=$(curl -s icanhazip.com)

#to get your external name of your network by going to the website then using awk to just display the external name of your network
en=$(getent hosts $(curl -s icanhazip.com) | awk '{print $2}')
